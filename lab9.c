#include <stdio.h>
#include <stdlib.h>

#define TABLESIZE 23

// RecordType
struct RecordType
{
	int		id;
	char	name;
	int		order; 
	struct RecordType* next;
};

// Fill out this structure
struct HashType
{
	struct RecordType* record;
};

// Compute the hash function
int hash(int x)
{
	return x % TABLESIZE;
}

// parses input file to an integer array
int parseData(char* inputFileName, struct RecordType** ppData)
{
	FILE* inFile = fopen(inputFileName, "r");
	int dataSz = 0;
	int i, n;
	char c;
	struct RecordType *pRecord;
	*ppData = NULL;

	if (inFile)
	{
		fscanf(inFile, "%d\n", &dataSz);
		*ppData = (struct RecordType*) malloc(sizeof(struct RecordType) * dataSz);
		// Implement parse data block
		if (*ppData == NULL)
		{
			printf("Cannot allocate memory\n");
			exit(-1);
		}
		for (i = 0; i < dataSz; ++i)
		{
			pRecord = *ppData + i;
			fscanf(inFile, "%d ", &n);
			pRecord->id = n;
			fscanf(inFile, "%c ", &c);
			pRecord->name = c;
			fscanf(inFile, "%d ", &n);
			pRecord->order = n;
		}

		fclose(inFile);
	}

	return dataSz;
}

// prints the record
void printRecords(struct RecordType pData[], int dataSz)
{
	int i;
	printf("\nRecords:\n");
	for (i = 0; i < dataSz; ++i)
	{
		printf("\t%d %c %d\n", pData[i].id, pData[i].name, pData[i].order);
	}
	printf("\n\n");
}

// display record in the hash structure
// skip the indices which are free
// the output will be in the format:
// index x -> id, name, order -> id, name, order ....
void displayRecordsInHash(struct HashType *pHashArray[], int hashSz)
{
	int i;

	for (i=0;i<hashSz;++i)
	{
		// if index is occupied with any record, print all
        if (pHashArray[i] != NULL && pHashArray[i]->record != NULL)
        {
            printf("Index %d:", i);
            struct RecordType *currentRec = pHashArray[i]->record;
            while (currentRec != NULL)
            {
                printf(" -> %d, %c, %d", currentRec->id, currentRec->name, currentRec->order);
                currentRec = currentRec->next;
            }
        printf("\n");
        }
	}
}

int main(void)
{
	struct RecordType *pRecords;
	int recordSz = 0;

	recordSz = parseData("input.txt", &pRecords);
	printRecords(pRecords, recordSz);
	// Your hash implementation
	// Hash table
    struct HashType *hashTable[TABLESIZE] = { NULL };
	
	// Insert records to hash table
    for (int i = 0; i < recordSz; i++)
    {
        int index = hash(pRecords[i].id);
        struct HashType *hashIndex = hashTable[index];
        struct RecordType *newNode = (struct RecordType*) malloc(sizeof(struct RecordType));

        *newNode = pRecords[i];
        newNode->next = NULL;
        
        if (hashIndex == NULL)
        {
            hashIndex = (struct HashType*) malloc(sizeof(struct HashType));
            if (hashIndex != NULL)
            {
            	hashIndex->record = newNode;
            	hashTable[index] = hashIndex;
            }
        }
        else
        {
            struct RecordType *currentRec = hashIndex->record;
            while (currentRec->next != NULL)
            {
                currentRec = currentRec->next;
            }
            currentRec->next = newNode;
        }
    }

    displayRecordsInHash(hashTable, TABLESIZE);

    // Free memory
    for (int i = 0; i < recordSz; i++)
    {
        struct RecordType *currentRec = pRecords[i].next;
        while (currentRec != NULL)
        {
            struct RecordType *tmp = currentRec;
            currentRec = currentRec->next;
            free(tmp);
        }
    }
    free(pRecords);

    // Free memory
    for (int i = 0; i < TABLESIZE; i++)
    {
        struct HashType *currentHash = hashTable[i];
        if (currentHash != NULL)
        {
            struct RecordType *currentRec = currentHash->record;
            while (currentRec != NULL)
            {
                struct RecordType *tmp = currentRec;
                currentRec = currentRec->next;
                free(tmp);
            }
            free(currentHash);
        }
    }

    return 0;
}